...
import { Menu, X, Globe, User, LogOut, LayoutDashboard, ChevronDown, CalendarDays } from 'lucide-react';
                    ...
                    <HMenu.Item>
                      {({ active }) => (
                        <Link
                          ...
                          className={cn('flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700', active && 'bg-gray-50')}
                        >
                          <CalendarDays size={15} /> {t('nav.bookings')}
                        </Link>
                      )}
                    </HMenu.Item>
                    {isAdmin && (
        ...
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 flex flex-col gap-1">
          ...
          {isAuthenticated ? (
            <>
              ...
              <Link to="/my-bookings" onClick={() => setMobileOpen(false)} className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
                <CalendarDays size={14} /> {t('nav.bookings')}
              </Link>
              {isAdmin && (
