    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Cancellation policy */}
          <div data-aos="fade-up" className="bg-amber-50 rounded-2xl p-4 border border-amber-200 flex items-center gap-2">
            <LucideIcons.ShieldAlert size={16} className="text-amber-600 flex-shrink-0" />
            <h2 className="font-semibold text-amber-800">{lang === 'ar' ? 'سياسة الإلغاء' : 'Cancellation Policy'}</h2>
          </div>
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div data-aos="fade-up" className="top-24 z-50 bg-white rounded-2xl border border-gray-200 shadow-md p-5 space-y-4">
            ...
            <a
              href={`https://wa.me/${WHATSAPP_NUMBER.replace(/\D/g, '')}?text=${encodeURIComponent(lang === 'ar' ? `مرحباً، أود الاستفسار عن ${chalet.name.ar}` : `Hello, I'm interested in booking ${chalet.name.en}`)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-[#25D366] hover:bg-[#20bc5a] text-white font-semibold text-sm transition-colors"
            >
            ...
            <p className="text-center text-xs text-gray-400">{lang === 'ar' ? 'لن يتم خصم أي مبلغ الآن' : "You won't be charged yet"}</p>
