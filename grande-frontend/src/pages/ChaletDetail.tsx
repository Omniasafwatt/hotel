...
import { Star, Users, Bed, Bath, ChevronLeft, ChevronRight, ExternalLink, MapPin, Plus, Minus } from 'lucide-react';
                ...
                <div className="space-y-2 max-h-56 overflow-y-auto">
                  {addons.map((addon) => {
                    ...
                    const priceLabel = addon.isPerNight
                      ? (lang === 'ar' ? `${addon.price} د.ك / ليلة` : `${addon.price} KWD / night`)
                      : `${addon.price} KWD`;
                    return (
                      <div
                        key={addon.id}
                        className={`flex items-center justify-between rounded-xl border px-3 py-2 transition-colors ${qty > 0 ? 'border-gold-400 bg-gold-50' : 'border-gray-100 bg-gray-50'}`}
                      >
                        <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 ms-2 flex-shrink-0">
                          ...
                          <button
                            ...
                            className="w-5 h-5 rounded-full bg-gold-100 hover:bg-gold-200 flex items-center justify-center text-gold-700 transition-colors"
                          >
                            <Plus size={10} />
                          </button>
                        </div>
