...
export function PricingSummary({ pricing, paymentPlan, chaletName, checkIn, checkOut, addonLines, className }: PricingSummaryProps) {
  const { t } = useTranslation();
  const addonsTotal = (addonLines ?? []).reduce((s, a) => s + a.price, 0);
  ...
  return (
    <div className={cn('bg-white rounded-2xl border border-gray-200 p-5', className)}>
      ...
      <div className="space-y-0.5 border-t border-gray-100 pt-3">
        ...
        {(addonLines ?? []).length > 0 && (
          <>
            <div className="border-t border-dashed border-gray-100 mt-1 pt-1" />
            {addonLines!.map((a) => (
              <Row key={a.name} label={a.name} value={`+${a.price} ${t('common.sar')}`} />
            ))}
          </>
        )}
        <div className="border-t border-gray-200 mt-2 pt-2">
          <Row
            ...
            value={`${grandTotal.toLocaleString()} ${t('common.sar')}`}
            bold
          />
        </div>
      </div>
      ...
      {paymentPlan === 'partial' && (
        <div className="mt-4 p-3 bg-gold-50 rounded-xl space-y-1.5">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-gold-700">{(grandTotal * 0.3).toLocaleString()} {t('common.sar')}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">{t('booking.remaining')}</span>
            <span className="text-gray-600">{(grandTotal * 0.7).toLocaleString()} {t('common.sar')}</span>
          </div>
        </div>
      )}

      <div className="mt-4 text-center text-xs text-gray-400">
        ≈ {pricing.perNightAverage.toLocaleString()} {t('common.sar')} {t('common.per_night')} avg. (room only)
      </div>
    </div>
  );
}
